...
private HuffmanNode root;
	...
	private Queue<HuffmanNode> createTree(Queue<HuffmanNode> root) {
		if (root.size() > 1) {
			...
			root.add(new HuffmanNode((char) 0, first.getFrequence() + second.getFrequence(), first, second));
	...
	private HuffmanNode constructTree(int asc, String code, HuffmanNode root) { 
		if (code.length() <= 1) {
			...
			if (root.leftZero == null) { // If node is null, create new node location for left
				root.leftZero = constructTree(asc, code.substring(1, code.length()), new HuffmanNode((char) 0, 0));
			} else { // If node is not null, go left to the next location
				root.leftZero = constructTree(asc, code.substring(1, code.length()), root.leftZero);
			}
		} else if (code.charAt(0) == '1' && code.length() > 1) {
			if (root.rightOne == null) { // If node is null, create new node location for right
				root.rightOne = constructTree(asc, code.substring(1, code.length()), new HuffmanNode((char) 0, 0));
			} else { // If node is not null, go right to the next location
				root.rightOne = constructTree(asc, code.substring(1, code.length()), root.rightOne);
			}
		}
		return root;
	}
